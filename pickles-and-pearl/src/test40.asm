...
;    added 128k Banks and switching - Whole games loads in 8:45
...
attributes               equ 5800h+(yscreenoffset*32)
...
world5 equ 7
          ...
          ; Bank 0 - We will put the title code, and big home screen demo
          ; Bank 1 - World 1
          ; Bank 2 - DO NOT USE ($8000)
          ; Bank 3 - World 2
          ; Bank 4 - World 3
          ; Bank 5 - DO NOT USE ($4000)
          ; Bank 6 - World 4
          ; Bank 7 - World 5

	org $6000

; We have 8k for graphics unexpanded giving us

; 576 Bytes per sprite (3 x 3 chars, 2 frames per direction) = 14 Sprites total available accross all worlds
...
sprite_unex_2: defs $240
sprite_unex_3: defs $240
sprite_unex_4: defs $240
sprite_unex_5: defs $240
sprite_unex_6: defs $240
sprite_unex_7: defs $240
sprite_unex_8: defs $240
sprite_unex_9: defs $240
sprite_unex_A: defs $240
sprite_unex_B: defs $240
sprite_unex_C: defs $240
sprite_unex_D: defs $240
...
begin:
     ...
     ld a,title                   ; Title Screen (Bank 0) is the Title area
     call switchbank
...
startnewgame:
     ...
     ; Copy the world keys to collect to the temporary buffer in $8000 memory
     ld hl,keystocollect
     ld de,keydata
     ld bc,$ff
     ldir
...
resetenergy:
     ld a,30
     ld (energy),a
     ld hl,attributes+768-31-(yscreenoffset*32)
...
cells_b
          ...
          push hl
          ld hl,scr_lookup+(yscreenoffset*64)
          add hl,bc
          add hl,bc
...
setup_screen
             ...
             ld hl,bg_buffer
             ld (hl),0
             ld bc,767*5 ; 2 Buffers and 3 sprites
             ldir
...
collide:
     ...
     ld iy,sprites    ;      get player
     ld a,(iy+1)      ;      player xpos
     rl a             ;    Multiply by 8
     rl a
     rl a
     ld e,a                ;  e has proper x pos
     ld a,(iy+4)      ; Add anim counter
     rl a              ; Rotate by 2 (2 pixels per frame)
     add a,e           ; store in e
     ld e,a
     

     ld a,(ix+1)         ; Do same with enemy
     rl a
     rl a
     rl a
     add a,(ix+4)
     ...
     sub e
     add a,20            ; 24 amd 48 were not as accurate
     cp 40
...
showkeys:
             ...
             ld hl,keydata+1  ; Offset for number of keys

             ld a,(keydata)   ; Total keys to collect
...
decreaseenergy:
    ...
    ld hl,attributes+768-30-(yscreenoffset*32)
    ld e,a
    ld d,0
    add hl,de
...
;sprite_unex_0 equ $c000
;sprite_unex_1 equ $c000+(144*4)
sprites equ $c000
enemy_sprites equ sprites+sprite_size
keystocollect equ $c100
bg_chars equ $c200
bg_chars_attrs equ $c600
blox equ $c700
; the main buffer, cleared every loop to tell what needs updating
; if it is 0 then the char is ignored
update_buffer:                equ $c800
...
bg_buffer:                   equ $c800+768

;  note: it is best to fill with colour of sprites, less flicker
back_colour_buffer:           equ $c800+768+768

; char buffer for each sprite, up to 6

; sprite buffer 1 is used by the player
sprite_buffer_1:             equ $c800+768+768+768

; others are used by sprites (1-2)
sprite_buffer_2:             equ $c800+768+768+768+768
sprite_buffer_3:             equ $c800+768+768+768+768+768

scratch equ $da00
keygfx equ $da00+8
screens equ $f300
sprite_expand_1 equ $db00
sprite_expand_2 equ $e700
...
showmenu:
     ...
     cp key_1
     jr nz,sm2
     ld a,world1                   ; Bank 1 contains level 1 (Although in game, 0 will have big demo)
...
sm2:
     cp key_2
     jr nz,sm3
     ld a,world2                   ; Bank 1 contains level 1 (Although in game, 0 will have big demo)
...
; tell assembler to start at 8000h
	...
	SAVESNA "src/test40.sna",main
	SAVETAP "src/test40.tap",main
